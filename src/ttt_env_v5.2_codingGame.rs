...
static ESTIMATE_VALUE: bool = false;
...
fn main() {
	...
	let mut agent = new_Agent(&env, 0.85, 1, overboard_evaluation, 90, 1000, false); //env, cpuct, nb_evaluation_sim, evaluation_method, act_timer_ms, nb_MCTS_simulations_max
	//use first loop to iterate a bit longer
	for _i in 0..10 {
		agent.act(&env);
	}
	...
	loop {
		...
		if opponent_row!= -1 {
			...
			let om = xy_to_gn(opponent_col as u8, opponent_row as u8);
			env.step(om);
			//run mcts (90ms)
			//eprintln!("{:?}", &env);
			(action, value, _) = agent.act(&env);
		}
		//simulate our move
		env.step(action);
		let (x,y) = gn_to_xy((action));
